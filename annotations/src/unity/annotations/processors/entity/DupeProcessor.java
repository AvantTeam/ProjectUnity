...
public class DupeProcessor extends BaseProcessor{
    ...
    public void process(RoundEnvironment roundEnv) throws Exception{
        ...
        if(round == 1){
            ...
            for(TypeElement comp : comps.select(t -> t.getEnclosingElement() instanceof PackageElement)){
                ...
                TypeElement buildType = null;
                for(int i = 0; i < types.size; i++){
                    //find the build type
                    if(this.types.isAssignable(
                            types.get(i).asType(),
                            toType(Building.class).asType())) buildType = types.get(i);
                }
                ...
                if(buildType != null){
                    ...
                    builder
                            ...
                            .addAnnotation(cName(DupeInterface.class));
                }
    ...
    DupeDefinition toClass(Element def, ObjectMap<String, Element> usedNames, boolean isBuild){
        ...
        TypeSpec.Builder builder = TypeSpec.classBuilder(name).addModifiers(Modifier.PUBLIC)/*.superclass(procName(isBuild ? findBuild(parentClass) : parentClass, DupeProcessor::simpleName))*/;
        ...
        for(ExecutableElement m : methods(comp).select(t -> !isConstructor(t))){
            ...
            methodBlocks.put(descString(m) + (isBuild ? "BUILD" : ""), annotation(m, Actually.class) != null ? annotation(m, Actually.class).code() : procBlock(trees.getTree(m).getBody().toString()).replaceAll(
        ...
        for(VariableElement field : fields){
            ...
            if(!isBuild && ttype.equals(tName(baseClass))){
                ttype = tName(def);
            }
    ...
    String baseName(TypeElement type){
        ...
        if(!name.endsWith("Comp")){
            throw new IllegalStateException("All types annotated with @DupeComponent must have 'Comp' as the name's suffix: '" + name + "'");
        }
