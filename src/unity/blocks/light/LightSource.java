...
public class LightSource extends GenericCrafter{
	...
	public LightSource(String name, boolean angleConfig){ this(name, false, angleConfig); }
	...
	public class LightSourceBuild extends GenericCrafterBuild{
		...
		protected void setInit(boolean a){ lightInit = a; }

		protected boolean initDone(){ return lightInit; }

		protected float getAngleDeg(){ return rotate ? rotdeg() : angle * 45f; }

		protected int getAngle(){ return rotate ? rotation * 2 : angle; }
		...
		public void addAngle(int a){ setAngle((angle + a + 8) % 8); }

		protected void setStrength(float a){ strength = a; }
			...
			for (int i = 0, len = ls.size(); i < len; i++){
				...
				if (i == len - 1 || ls.get(i + 1) == null){
					...
					Drawf.tri(tempX + ((tempLength - 1) * tilesize + shift) * Geometry.d8(tempAngle).x,
						tempY + ((tempLength - 1) * tilesize + shift) * Geometry.d8(tempAngle).y, w,
						tempAngle % 2 == 0 ? 8f : 11.313f, tempData.angle * 45f);
				}else{
					Tile nextTile = ls.get(i + 1);
					if (lsData.get(i + 1) == null)
						Lines.line(tempX, tempY, nextTile.worldx() - 4 * Geometry.d8(tempAngle).x,
							nextTile.worldy() - 4 * Geometry.d8(tempAngle).y);
