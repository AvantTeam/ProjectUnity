...
public class WormSegmentUnit extends UnitEntity{
	...
	@Override
	public void damage(float amount){ trueParentUnit.damage(amount); }

	@Override
	public void damage(float amount, boolean withEffect){ trueParentUnit.damage(amount, withEffect); }
	...
	@Override
	public int classId(){ return UnityUnitTypes.getClassId(1); }
	...
	public void setSegmentType(int val){ segmentType = val; }
	...
	protected void updateWeapon(){
		...
		for (int i = 0; i < mounts.length; i++){
			...
			float offset = Angles.trnsx(rotation - 90, weapon.x, weapon.y)
				+ Angles.trnsx(weaponRotation, weapon.shootX, weapon.shootY);
			float shootX = x + offset;
			float shootY = y + offset;
			float shootAngle = weapon.rotate ? weaponRotation + 90f
				: Angles.angle(shootX, shootY, mount.aimX, mount.aimY) + rotation - angleTo(mount.aimX, mount.aimY);
			...
			if (weapon.rotate && (mount.rotate || mount.shoot) && can){
				...
				mount.rotation = Angles.moveToward(mount.rotation, mount.targetRotation,
					weapon.rotateSpeed * Time.delta);
			}else if (!weapon.rotate){
				mount.rotation = 0;
				mount.targetRotation = angleTo(mount.aimX, mount.aimY);
			}
			if (mount.shoot && can && (ammo > 0 || !state.rules.unitAmmo || team().rules().infiniteAmmo)
				&& (!weapon.alternate || mount.side == weapon.flipSprite)
				&& (vel.len() >= mount.weapon.minShootVelocity || (net.active() && !isLocal()))
				&& mount.reload <= 1.0E-4F && Angles.within(weapon.rotate ? mount.rotation : rotation,
					mount.targetRotation, mount.weapon.shootCone)){
	...
	protected void shoot(WeaponMount mount, Weapon weapon, float x, float y, float aimX, float aimY, float rotation,
		...
		}else Angles.shotgun(weapon.shots, weapon.spacing, rotation,
			f -> mount.bullet = bullet(weapon, x, y, f + Mathf.range(weapon.inaccuracy), lifeScl));
	...
	protected Bullet bullet(Weapon weapon, float x, float y, float angle, float lifescl){
		return weapon.bullet.create(this, this.team, x, y, angle,
			1.0f - weapon.velocityRnd + Mathf.random(weapon.velocityRnd), lifescl);
	}
