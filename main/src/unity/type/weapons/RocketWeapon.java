...
public class RocketWeapon extends LimitedAngleWeapon{
    ...
    public void drawOutline(Unit unit, WeaponMount mount){
        ...
        if(rMount.warmup > 0.01f || loadf(mount) > 0.01f){
            if(outlineRegion.found() && !top){
                Draw.draw(Draw.z(), () -> {
                    UnityDrawf.vConstruct(wx, wy, outlineRegion, weaponRotation, loadf(mount), rMount.warmup, rMount.total);
                });
            }
        }else{
    ...
    public void draw(Unit unit, WeaponMount mount){
        ...
        if(rMount.warmup > 0.01f || loadf(mount) > 0.01f){
            Draw.draw(Draw.z(), () -> {
                ...
                UnityDrawf.vConstruct(wx, wy, region, weaponRotation, loadf(mount), rMount.warmup, rMount.total);
            });
        }else{
            if(outlineRegion.found() && top){
                Draw.rect(outlineRegion,
    ...
    public static class RocketMount extends LimitedAngleMount{
        float total, warmup;
