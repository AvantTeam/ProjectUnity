...
public class IconBar extends Element{
    ...
    public void draw(){
        ...
        for(int i = 0, len = data.values.length; i < len; i++){
            ...
            Draw.rect(data.icons[i], dPos, y + ah * 0.75f);
        }
        ...
        for(float i = stsrt; i < maxVal; i += stepSize){
            ...
            font.draw(text, dPos - lay.width / 2f, y + ah * 0.25f + lay.height / 2f + 1f);
        }
        Pools.free(lay);
    }
