...
public class CrucibleGraph extends BaseGraph<GraphCrucibleModule, CrucibleGraph>{
    ...
    CrucibleGraph create(GraphCrucibleModule module){
        return new CrucibleGraph(module);
    }
    ...
    /*CrucibleData getMelt(Item meltProd){
        return contains.find(i -> i.id == meltProd.id);
    }*/
    //TODO
    boolean addMeltItem(MeltInfo meltProd, float am, boolean liquid){
    ...
    float getRemainingSpace(float amount){
    ...
    void addLiquidToSlot(CrucibleData slot, float am){
        if(am <= 0f) return;
    ...
    void updateOnGraphChanged(){
        ...
        for(var module : connected){//building
            int bitmask = 0;
            if(module.parent.build.tile() == null){
            ...
            for(int i = 0; i < 8; i++){
                ...
                GraphCrucibleModule conModule = (GraphCrucibleModule)build.getGraphConnector(GraphType.crucible);//conBuild
