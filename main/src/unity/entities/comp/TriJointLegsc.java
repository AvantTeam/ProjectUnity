...
public interface TriJointLegsc extends Unitc{
    TriJointLeg[] legs();
    void legs(TriJointLeg[] triLegs);

    float baseRotation();
    void baseRotation(float v);

    float moveSpace();
    void moveSpace(float v);

    float totalLength();
    void totalLength(float v);
    ...
    default float legAngle(float rotation, int index){
    ...
    default void update(){
        ...
        totalLength(totalLength() + Mathf.dst(deltaX(), deltaY()));
