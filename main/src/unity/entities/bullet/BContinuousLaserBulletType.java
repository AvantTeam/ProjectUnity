...
//temporary naming
public class BContinuousLaserBulletType extends ContinuousLaserBulletType{
    public BContinuousLaserBulletType(float damage){
        super(damage);
    }

    public BContinuousLaserBulletType(){
        this(0f);
    }
    ...
    public void update(Bullet b){
        if(!(b.data instanceof BData)) b.data = new BData(0f, 3, b.rotation(), 10);
        BData temp = (BData) b.data;
    ...
    public void draw(Bullet b){
        if(!(b.data instanceof BData)) return;
        BData temp = (BData) b.data;
        float realLength = Damage.findLaserLength(b, temp.f);
        float fout = Mathf.clamp(b.time > b.lifetime - fadeTime ? 1f - (b.time - (lifetime - fadeTime)) / fadeTime : 1f);
        float baseLen = realLength * fout;
