        ...
        addPattern(new KamiPattern(){{
            ...
            stages = new KamiPatternStage[]{
                new KamiPatternStage(5f * 60f, ai -> {
                    ...
                    float diffReload = 3f - Mathf.clamp(ai.difficulty / 1.75f, 0f, 2f);
                    ...
                    if(ai.reloads[3] >= diffReload){
                        for(int i = 0; i < diff; i++){
                            ...
                            tVec.trns(angle1, fout1 * 70f).add(ai);
                            ...
                            if(fout2 > 0f){
                                tVec.trns(angle2, fout2 * 70f).add(ai);
                                ...
                                if(ai.difficulty >= 3) KamiBulletPresets.shootLine(type, ai.unit, ai.unit.team, tVec.x, tVec.y, angle2, type.speed * 0.15f, (type.speed * 0.3f) + extraSpeed, diff2, b -> b.hitSize *= 0.75f);
                            }
                        }
                        ai.reloads[3] = 0f;
                        ai.reloads[4] += diffReload * 5.8f * ai.reloads[0];
                    }
                }),
