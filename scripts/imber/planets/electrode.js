const electrodeGen = extend(PlanetGenerator, {
	rawHeight(pos){
	...
	generate(){
		...
		if(this.simplex.octaveNoise3D(2, 0.5, scl, this.sector.tile.v.x + 2, this.sector.tile.v.y, this.sector.tile.v.z) * nmag + poles > 0.6 * addscl){
			ores.add(Vars.content.getByName(ContentType.block, "unity-imberium"));
		};
...
electrodeGen.waterOffset = 0.07;
electrodeGen.scl = 6;
electrodeGen.rid = new Packages.arc.util.noise.RidgedPerlin(1, 2);
electrodeGen.basegen = new BaseGenerator();
...
electrode.atmosphereColor = Color.valueOf("ada41c");
